...
namespace FinalPorjectDatabaseFirst3.Controllers
{
    public class UsersController : Controller
    {
        ...
        public async Task<ActionResult> Create([Bind(Include = "UserID,UserName,UserPassword,UserType")] TblUser tblUser)
        {
            if (ModelState.IsValid)
            {
                ...
                oTblUser.UserPassword = tblUser.UserPassword;
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<ActionResult> Edit([Bind(Include = "UserID,UserName,UserPassword,UserType")] TblUser tblUser)
        {
            if (ModelState.IsValid)
            {
        ...
        public async Task<ActionResult> UserRole(int userId)
        {
            ...
            #region Customers
            oUserRole = listTblUserRole.Where(o => o.UserID == userId && o.PageName == "Routes").FirstOrDefault();
            if (oUserRole == null)
            {
                oUserRole = new TblUserRole();
                oUserRole.UserID = userId;
                oUserRole.PageName = "Routes";
                oUserRole.IsCreate = false;
                oUserRole.IsRead = false;
                oUserRole.IsUpdate = false;
                oUserRole.IsDelete = false;
